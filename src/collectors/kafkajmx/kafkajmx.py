...
CLUSTER = ['cluster0', 'cluster1']  # kafka clusters, make sure exists in ZK
ZKS = 'zk:2181'  # zk server, e.g.('zk1:2181,zk2:2181,zk3:2181')
USER, PASS = "", "" # JMX User/Password
JVM_PATH = os.path.join(os.environ.get('JAVA_HOME'), 'jre/lib/amd64/server/libjvm.so')  # jvm path
Metric = namedtuple(
    typename='Metric',
    field_names=(
        'name',      
        ...
        try:
            ...
            for server in brokers:
                for k, v in self.poll_metrics(METRICS):
                    stat = 'stats.gauge.kafka.%s.%s' % (server, k)  # customize stats prefix here
